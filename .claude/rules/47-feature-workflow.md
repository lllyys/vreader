...
- **Wiring-claim verification** — every statement the plan makes about *existing* code must be checked against the actual codebase, not taken on the author's word. Any assertion that an entry point, route, menu item, or integration "is wired" / "is reachable" is verified by finding the call site; an unverified wiring claim is a **High** finding. Precedent: `dev-docs/plans/20260622-feature-118-android-ai-provider-chat.md:53-55` ("A production Settings entry is wired (the provider list is reachable)") and `dev-docs/plans/20260626-feature-120-android-opds-ui.md:39-40` ("a production Settings/Library entry point is wired") were both false when written; both features shipped `VERIFIED` with UI a user cannot open. #114's Gate-2 audit *did* catch this class as a High finding — the auditor can find it when the prompt asks for it.
...
**Production reachability (binding, both platforms).** A behavioral WI's Gate-5 pass must exercise the feature through a **production entry point in a release-configured build** — the path a real user takes, starting from app launch and navigating the shipped UI. DEBUG-source-set launchers (`android/app/src/debug/`, excluded from the release APK), `vreader-debug://` DebugBridge commands, and tests that invoke a composable / View / screen directly are **setup mechanisms, not entry points**: they may seed state, but they do not by themselves demonstrate that a user can reach the feature. iOS is not exempt — a DebugBridge-only verification has exactly the same hole. The evidence file must name the user-visible path taken (e.g. "Library → gear → Backup"), not just the harness command. **If no production path exists, the row is capped at `DONE` and may NOT reach `VERIFIED`** — the missing entry point is filed as a tracked blocker (a rule-51 `needs-design` issue when the gap is an undesigned surface, otherwise a feature/bug row) and referenced from the row before the feature is called complete. Precedent: Android features #114 (backup UI), #118 (AI chat/provider list), #120 (OPDS UI), and #122 (stats dashboard) were each implemented, instrumented-tested, audited, merged, flipped to `VERIFIED`, and had their GH issues closed while ten top-level composables had **zero** production call sites in `android/app/src/main/kotlin` — instrumented tests plus a `android/app/src/debug/` launcher satisfied the old bar. `scripts/check-orphan-surfaces.sh` is a *detector* for this class (it flags top-level composables with no production call site), not an enforcement hook — the discipline is the gate.
...
- `VERIFIED` is a separate post-implementation status, set after Gate 5's final-WI acceptance pass lands and is recorded in the row. Requires a `dev-docs/verification/feature-<id>-<YYYYMMDD>.md` evidence file (PreToolUse hook enforces) **and** a production-reachable path named in that evidence file (Gate 5 "Production reachability" — no hook enforces this one; a feature with no production entry point stays at `DONE`).
