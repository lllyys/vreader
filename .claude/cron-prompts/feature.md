...
DISPATCH SELF-GATE (feature #130 WI-7): the bugfix cron cut over to /dispatch first; this cron follows only after the harness proves itself there. First take the reentry guard `scripts/agent-lock.sh acquire cron-feature` (BLOCKED → log `blocked` and stop; release before ENDED on every exit path). Then count clean dispatch-mode iterations: `grep -c "ENDED work_done dispatch-mode" .claude/cron-logs/bugfix.log`. If the count is >= 3 AND `.claude/state/dispatch-kill` does NOT exist, you MAY run the selected feature's next Gate-3 WI through `/dispatch` (single lane — feature WIs are sequential within a feature; the skill takes the global `dispatch` lock itself; if it reports the dispatch lock BLOCKED, log `blocked (dispatch busy)` and stop — never run the WI inline while another dispatch is orchestrating). Otherwise run the WI inline via /feature-workflow exactly as before — the self-gate failing is normal, not an error. In BOTH modes, Gates 1/2 (plan + plan audit) and Gate-5b final acceptance stay inline in this session; /dispatch only runs Gate-3/4 lanes.
