...
struct DiagnosticsLevelStyleTests {
    ...
    @Test func groupsEntriesIntoTodayAndYesterdayNewestFirst() {
        ...
        let sections = DiagnosticsDayGrouper.sections(
            from: [entry(.info, at: todayA), entry(.error, at: yesterday), entry(.debug, at: todayB)],
        ...
        #expect(sections[0].entries.map(\.date) == [todayB, todayA])
    ...
    @Test func olderDayHasNoRelativeWord() {
        ...
        let sections = DiagnosticsDayGrouper.sections(from: [entry(.info, at: old)], now: now, calendar: cal)
