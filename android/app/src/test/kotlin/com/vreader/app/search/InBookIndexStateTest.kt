 ...
 * predicate MIRRORS [SearchIndexCoordinator.isEligible]/needsIndexing: a MISSING row, a row at an OLD
 * indexerVersion, or a `failed` row is still-working (except `failed` surfaces as retryable Failed); a
 * current-version `indexed`/`skipped_unsupported` row is settled.
 */
...
class InBookIndexStateTest {
    ...
    @Test fun txt_unexpectedStatus_isIndexing() {
        // An in-progress / typo / unknown status is NOT a settled terminal → hold as Indexing (mirrors the
        // DAO's defensive NOT IN(indexed, skipped_unsupported) completeness predicate).
        for (bogus in listOf("indexing", "faild", "pending", "")) {
            assertEquals(
                "status='$bogus' must be treated as still-working",
                InBookIndexState.Indexing,
    ...
    @Test fun txt_failedStaleVersion_isFailed() {
        // A failed row is retryable regardless of version — surface Failed, not the generic Indexing.
        assertEquals(
            InBookIndexState.Failed,
