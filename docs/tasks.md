...
2026-05-15 (Feature #31 round-4 verify, ambiguous cause — triage needed): when launching `--uitesting --seed-md-multi-page --reader-default-layout=paged` and tapping the seeded "Test Markdown Multi-Page" book card on iPhone 17 Pro Sim with computer-use clicks, the vreader process cleanly terminates and iOS returns to the home screen — no crash report under `~/Library/Logs/DiagnosticReports/`, no SIGABRT / Precondition / fatalError trace in `xcrun simctl spawn booted log show --predicate 'process == "vreader"'`. Reproducible across 3 clicks at slightly different coords inside the card. Separately, the DebugBridge `vreader-debug://open?bookId=md:000…c0c002:9231` URL (both raw + URL-encoded variants) returns no error AND `lastError: null` in the post-snapshot, but `currentBookId` stays `null` despite ≥4s wait — i.e. the open command doesn't surface state through the snapshot probe. Both symptoms recorded in `dev-docs/verification/feature-31-20260515-round4.md`. **Triage question**: is this (a) a Debug-build silent abort in the MD-paged-mode book-open path (Bug — file under bugs.md), (b) a DebugBridge wiring gap where the MDReaderContainerView probe doesn't register `currentBookId` (Feature #45 verification-harness follow-up — file under features.md), or (c) a computer-use coords-routing artifact (tooling — note + move on)? Need to triage before classifying.
