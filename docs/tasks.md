...
_(empty — inbox clear as of 2026-06-16)_


## Triaged

2026-06-16 | NO-ACTION (resolved by subsequent work) | The 2026-05-17 feature-cron item flagging a Feature #42 (Foliate-js unified reader engine) tracker inconsistency (row PLANNED vs GH #113 CLOSED/COMPLETED) is now **moot**: row #42 is **VERIFIED** (re-scoped to "Readium + libmobi convert-on-import"; GH #113 correctly CLOSED COMPLETED), and the dependent rows #54 + #57 are both **VERIFIED**. The 2-week-old closure was NOT stale — #42's scope was reconciled/narrowed and completed in the intervening month. No action needed; the cron-blocking concern (#42 was "the only PLANNED feature") no longer applies.

2026-06-16 | NO-ACTION (already resolved inline) | The 2026-05-15 Feature #31 round-4 item resolved same-day to **Bug #191** (GH #682) — `AutoPageTurner.interval` didSet infinite recursion under `@Observable` (~23.7k-frame stack-guard fault in the reader-open path; the DebugBridge `currentBookId: null` was a downstream effect of the pre-registry crash). Fixed via `_intervalRaw` + computed clamping setter. Moved out of New for hygiene.
