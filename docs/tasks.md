...
2026-06-08 (verification-discovered, feature #68 Gate-5 round 3) — EPUB chapter-start drop-cap is absent in continuous-scroll mode. The feature #68 drop-cap CSS rule (`body > p:first-of-type::first-letter`, oxblood 2.6× float-left) IS injected into the stylesheet, but in the #71 legacy continuous-scroll stitch the `<p>` elements are wrapped inside `#vreader-scroll-root` + per-spine `[data-vreader-spine-index]` DIVs, so `body > p:first-of-type` (a direct-child combinator) matches nothing → no drop-cap renders. Eval proof on `mini-epub3` (v3.59.23): `::first-letter` computes to `rgb(29,26,20)` / `16px` / `float:none` (body defaults), `document.querySelector('body > p:first-of-type')` returns null, `firstChildTag: DIV`, `dropCapRuleInCSS: true`. Continuous scroll is a primary EPUB reading mode; the prior verification round only confirmed the drop-cap in a paged/single-section render where `<p>` is a body child. Likely the same selector also misses the Foliate (AZW3) windowed DOM. Expected: drop-cap on the first paragraph's first letter at each chapter start in continuous mode. Actual: no drop-cap. Suspected fix: broaden the selector to match the wrapped DOM (e.g. `[data-vreader-spine-index] > p:first-of-type::first-letter`) or inject per-section. Evidence: `dev-docs/verification/feature-68-20260608.md`.
