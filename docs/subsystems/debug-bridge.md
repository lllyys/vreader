...
| ---------- | ---------------------------------- | ------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `reset`    | —                                  | —                                     | Clean slate for verification: wipe every book from the library, wipe the FTS search index, AND clear persisted reader settings (`ReaderSettingsStore.allPersistedDefaultsKeys`, Bug #272) so the next reader open reads deterministic defaults — a leaked `readerAutoPageTurn=true` previously made the paged reader auto-advance on open and pollute `open?position=` snapshots. Idempotent.                                                                                                                       |
...
| `navigate` | `spine=<int≥0>`                    | `fraction=<0...1>`                    | Drive `.readerNavigateToLocator` for the active **EPUB** reader CU-free (Bug #273) — the verification harness for feature #71 WI-8 continuous-mode TOC/bookmark/search navigation, which the `search` driver cannot exercise in continuous mode (it paints no highlight / does not scroll there). Posts `.debugBridgeNavigateCommand`; the live `EPUBReaderContainerView` observer resolves `spine` → its spine `href` against `viewModel.metadata`, builds a `Locator` with the open book's fingerprint, and re-posts `.readerNavigateToLocator` — the SAME channel a TOC/bookmark/search tap uses, so the WI-8 coordinator (continuous) or the legacy paged path performs the jump. `spine` is required (non-negative; the observer additionally range-checks against the loaded spine count); `fraction` is optional (finite, clamped `0...1`; absent ⇒ chapter start). Use the `multi-chapter-epub` fixture (4 viewport-tall chapters) so the scroll is measurable AND the out-of-window rebuild branch is reachable (anchor 0 → window `[0,1]`; `navigate?spine=3` rebuilds to `[2,3]`). No-op when no matching EPUB reader is loaded (mirrors `seek`/`search`/`present`). |
| `locate`   | `highlight=<int≥0>`                | —                                     | Drive `.readerNavigateToLocator` for the active **TXT/MD** reader CU-free so the **locate "bloom"** (feature #74 — a ~1.5s sub-second wash-lift on highlight/note landing) can be asserted. The bloom VISUAL cannot be screenshot/video-captured on the Screen-Sharing virtual display (recordVideo has a ~3.5s capture-startup delay; TXT renders blank intermittently in `simctl io`), so this command fires the bloom through the **real render path** and the DEBUG `snapshot` reads it back via `landingBloomCount` / `landingBloomPeakIntensity` (DebugSnapshot schema v3). The handler resolves the `highlight`-indexed persisted highlight for the active book (`PersistenceActor.fetchHighlights`, newest-first — the SAME order the annotations sheet shows) and posts its saved `Locator` on `.readerNavigateToLocator` — the SAME channel a Notes/Highlights row tap uses, so the TXT container sets `uiState.highlightRange` + bumps `highlightNonce` and the bloom fires. `highlight` is required (non-negative integer; the Nth highlight, 0-based). Asserting: `snapshot` BEFORE shows `landingBloomCount==0`; `locate?highlight=0` → `settle` → `snapshot` shows `landingBloomCount>=1` + `landingBloomPeakIntensity>0.4` (above the resting 0.4 wash). No-op when no TXT/MD reader is presented or no Nth highlight exists (mirrors `navigate`/`seek`/`present`). |
| `scroll-boundary` | `spine=<int≥0>`, `near=top\|bottom` | —                            | Drive `EPUBContinuousScrollCoordinator.handleBoundarySignal(_:)` for the active **continuous-mode EPUB** reader CU-free (feature #71 WI-6b) — the verification harness for the scroll-driven window extension + eviction. The production `continuousScrollObserverJS` is rAF-throttled and rAF is paused on the headless/virtual-display test environment, so a synthetic touch scroll never fires a boundary report; this posts the signal directly. Posts `.debugBridgeScrollBoundaryCommand`; the live `EPUBReaderContainerView` observer builds the SAME `EPUBScrollBoundarySignal` the JS observer would (`intraFraction` 1.0 at the bottom / 0.0 at the top; `nearTopBoundary` / `nearBottomBoundary` set from `near`) and calls `coordinator.handleBoundarySignal` — re-entering the SAME WI-6b extension path a real scroll boundary hits (no parallel logic). `spine` is required (non-negative — same validation as `navigate`'s `spine`) and is the visible spine index; `near` is required and is which materialized-doc boundary the viewport is near (`top` ⇒ extend backward, `bottom` ⇒ extend forward). Continuous mode only (`continuousScrollConfig != nil`); no-op when no matching continuous-mode EPUB reader is loaded (mirrors `navigate`/`seek`/`search`/`present`). |
| `pdf-highlight` | `page=<int≥0>`, `rect=<x,y,w,h>` | `color=yellow\|pink\|green\|blue` | Create a PDF highlight for the active **PDF** reader CU-free (feature #17) — drives the selection-driven highlight → `PDFAnnotation` render + persist WITHOUT a real long-press-drag text selection (which needs a real touch / CU, unavailable on the virtual-display test environment). Posts `.debugBridgePDFHighlightCommand`; the live `PDFReaderContainerView` observer builds a `ReaderSelectionEvent` whose anchor is `AnnotationAnchor.pdf(page:, rects:)` (the rect denormalized `0...1` → page space downstream by `PDFAnnotationBridge.createHighlightFromAnchor`) and calls the SAME `handleHighlightAction(event:container:)` the gesture's "Highlight" confirmation triggers — coordinator → `PersistenceActor.addHighlight` → `PDFHighlightRenderer.apply` → `PDFAnnotationBridge.createHighlightFromAnchor` — so the annotation renders AND persists through the production seam (no parallel routine; the bridge-created highlight is byte-identical to a gesture-created one at the same `(page, rect)`). `page` is required (non-negative); `rect` is required and is the normalized highlight rect in page coordinate space (four comma-separated finite Doubles `x,y,w,h`, each `0...1`); `color` is optional (one of the four `NamedHighlightColor` rawValues, same allowlist as `highlight`; absent ⇒ yellow). Format-scoped to the PDF host (EPUB/TXT/MD/AZW3 don't register the observer); no-op when no PDF reader is loaded (mirrors `highlight`/`navigate`/`seek`/`present`). |
| `bilingual` | `action=enable\|disable\|status` | `lang=<str>`, `granularity=paragraph\|sentence` (`enable`), `dest=<basename>` (`status`) | Drive interlinear bilingual mode CU-free for the **EPUB / AZW3** readers (Feature #77 Gate-5b loading-shimmer verification). `enable` **bypasses the first-enable setup sheet** + the AI-provider gate (which an automated run can't satisfy) and runs the per-host enumerate→prefetch the More-menu toggle would — so the inline loading shimmer renders. Posts `.debugBridgeBilingualCommand`; whichever bilingual host is presented observes it: Readium (default EPUB engine) → `runBilingualEnumerateForCurrentChapter`; Foliate (AZW3/MOBI) → `bilingualOrchestrator.enumerateJS`; legacy EPUB → paged `pendingHighlightJS` / continuous `enableBilingualContinuousAllSections`. `disable` turns bilingual off + clears decorations. `status` writes `{engine,hasVM,isEnabled,aiConfigured,target,granularity,inFlight}` to `Caches/DebugBridge/<dest>` (default `bilingual-status.json`) — `inFlight>0` is the shimmer's source signal. Pair with `--mock-ai --mock-ai-translate-delay-ms=<N>` so the translate stays in-flight long enough to snapshot the shimmer before it lands. No-op when no bilingual EPUB/AZW3 reader is presented. |
...
- `highlight` (`locate`): the 0-based index of the persisted highlight to land on (newest-first, the same order the annotations sheet shows). Required; must be a non-negative integer (empty throws `parse.missingParam: highlight`; negative / non-integer throws `parse.invalidParam: highlight`) — same validation as `navigate`'s `spine`. The handler additionally range-checks against the active book's highlight count downstream (an out-of-range index is a silent no-op — no `.readerNavigateToLocator` post).
- `spine` (`scroll-boundary`): the visible spine index. Required; must be a non-negative integer (empty throws `parse.missingParam: spine`; negative / non-integer throws `parse.invalidParam: spine`) — same validation as `navigate`'s `spine`.
- `near` (`scroll-boundary`): one of `top` / `bottom`. Required and non-empty (empty throws `parse.missingParam: near`); anything else throws `parse.invalidParam: near`. `top` ⇒ extend backward, `bottom` ⇒ extend forward.
- `action` (`bilingual`): one of `enable` / `disable` / `status`. Required (empty throws `parse.missingParam: action`); anything else throws `parse.invalidParam: action`.
- `lang` / `granularity` (`bilingual` `enable`): optional. `lang` is a target-language key; `granularity` one of `paragraph` / `sentence` (an unrecognized granularity is ignored, keeping the persisted/default). Empty values are normalized to nil (keep the persisted setting).
- `dest` (`bilingual` `status`): optional readout basename; defaults to `bilingual-status.json`. Same path-safe charset as `token`/`dest`/`bridge`.
- `page` (`pdf-highlight`): the 0-based PDF page index. Required; must be a non-negative integer (empty throws `parse.missingParam: page`; negative / non-integer throws `parse.invalidParam: page`). The observer additionally range-checks against the loaded page count downstream (an out-of-range page produces no annotation — `PDFAnnotationBridge.createHighlightFromAnchor` returns `[]`).
- `rect` (`pdf-highlight`): the normalized highlight rect `x,y,w,h` in page coordinate space. Required; must be exactly four comma-separated finite Doubles, each in `0...1` (wrong component count / non-numeric / non-finite / out-of-range each throw `parse.invalidParam: rect`). The rect must also fit on the page (`x + w ≤ 1`, `y + h ≤ 1` — overflow throws `parse.invalidParam: rect`) and have **strictly positive area** (`w > 0` AND `h > 0` — a zero-area `w=0`/`h=0` rect throws `parse.invalidParam: rect`; a real text selection always covers glyphs and so has positive area, and `PDFAnnotationBridge.createHighlight` would otherwise persist an invisible record). The full-page boundary (`x+w == 1` / `y+h == 1`) is accepted. **Faithfulness (Codex Gate-4 round-2):** the observer derives `selectedText` from the **live, already-unlocked `PDFDocument`** the reader is showing (read via the renderer's `document`, bound by `PDFViewBridge`) — not a fresh `PDFDocument(url:)` reload — and creates the highlight **only when real glyphs sit under the rect**. A rect over blank page space (empty / whitespace selection) is a **no-op + log**, no highlight — mirroring the gesture path, which never posts `.readerTextSelected` for an empty selection.
- `color` (`pdf-highlight`): optional; one of `yellow` / `pink` / `green` / `blue` (the four `NamedHighlightColor` rawValues, same allowlist as `highlight`). Empty `color=` throws `parse.invalidParam: color`; an unknown value throws `parse.invalidParam: color`. Absent ⇒ the observer falls back to `"yellow"`.
...
| `vreader/Services/DebugBridge/RealDebugBridgeContext+Locate.swift` | `locate` command handler (feature #74 — resolves the Nth persisted highlight + posts its `Locator` on `.readerNavigateToLocator` so the locate bloom fires) |
| `vreader/Views/Reader/ReaderContainerView+DebugBridgeLandingBloom.swift` | Reader-host `.debugBridgeLandingBloomChanged` observer — caches `HighlightableTextView`'s bloom counters onto `DebugReaderProbeAdapter.landingBloomProbe` for the snapshot (feature #74) |
| `vreader/Services/DebugBridge/RealDebugBridgeContext+ScrollBoundary.swift` | `scroll-boundary` command handler (feature #71 WI-6b — posts `.debugBridgeScrollBoundaryCommand`) |
| `vreader/Views/Reader/EPUBReaderContainerView+DebugBridgeScrollBoundary.swift` | EPUB host `.debugBridgeScrollBoundaryCommand` observer — builds `EPUBScrollBoundarySignal`, calls `coordinator.handleBoundarySignal` (feature #71 WI-6b) |
| `vreader/Services/DebugBridge/RealDebugBridgeContext+PDFHighlight.swift` | `pdf-highlight` command handler (feature #17 — posts `.debugBridgePDFHighlightCommand`) |
| `vreader/Views/Reader/PDFReaderContainerView+DebugBridgePDFHighlight.swift` | PDF host `.debugBridgePDFHighlightCommand` observer — builds the `.pdf`-anchor `ReaderSelectionEvent`, calls the SAME `handleHighlightAction` the gesture uses (feature #17) |
