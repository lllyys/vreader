...
| 46 | WebDAV backup includes book files — materializing restore reconstructs library | Backup/* | High | IN PROGRESS | Phase 1 of two-feature split. Adds `library-manifest.json` to the backup ZIP and uploads missing whole-file book blobs to `VReader/books/<format>/<sha256>_<byteCount>.<ext>` (content-addressed, deduped). Restore downloads every missing blob into the sandbox before applying metadata. Preserves the existing invariant that a `Book` row always points to a readable local file — no `BookFileState`, no remote-only rows, no UI redesign. Implementation plan (post 2× Codex audit) at `dev-docs/plans/20260503-feature-46-materializing-restore.md`. Phase 2 = feature #47. GH: #144 |
