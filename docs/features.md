...
| 49 | DebugBridge probe completion (a) — registry / open / snapshot wiring | DevTools/* | High | PLANNED | Phase-A of the split: keyed `awaitReader(fingerprintKey:timeout:)` registry primitive (with token-based waiter ownership), `open(bookId:position:)` validation + native-mode position resolver (no per-format render seek), snapshot schema additions for selection/render-phase/settings-provenance fields, and `evalUnsupported(bridge:)` JSON shape. **No per-format host changes** — preserves the existing `DebugReaderProbe` adopters. Plan **v4** at `dev-docs/plans/20260503-feature-48-debugbridge-probe-completion.md` is implementation-ready: Round-3 surface fixes applied (CheckedThrowingContinuation, weak `activeReader`, `open()` typed against real `BookRecord.fingerprint.format`, live-read `positionProvider`, existing `eval-<bridge>.json` filename). Per audit-cap rule, no Round-4 audit needed — v4 is pure compile-correctness against verified live code. Ready for Gate 3. GH: #171 |
