...
| 46 | WebDAV backup includes book files — materializing restore reconstructs library | Backup/* | High | DONE | Phase 1 of two-feature split shipped. Data layer end-to-end: `library-manifest.json` (WI-2/6) + content-addressed blobs at `VReader/books/<format>/<sha256>_<byteCount>.<ext>` via temp+MOVE (WI-1/3/4) + `BookFileMaterializer` (WI-5) + provider backup/restore integration (WI-7/8) + production wiring (WI-8b). 11 PRs (#153–#163, v3.10.20→v3.10.30). 80+ new tests across 8 test files. Plan (post 2× Codex audit) at `dev-docs/plans/20260503-feature-46-materializing-restore.md`. Pending Gate 5 device verification (full backup→wipe→restore round-trip on iPhone 17 Pro Simulator) before status moves to VERIFIED. UI polish (size-confirm dialog when missing-bytes > 100 MB, per-blob progress) deferred as a follow-up. Phase 2 = feature #47. GH: #144 |
