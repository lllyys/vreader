...
| 48 | DebugBridge probe completion — settle hooks, live eval, richer snapshots, fixture path | DevTools/* | High | TODO (audit-cap) | Phase-1 prerequisite for feature #45. Plan went through 3 Codex audit rounds (cap per `.claude/rules/47-feature-workflow.md`). Plan v3 still has High-severity findings (host probe injection API, `open()` sketch references wrong types, `evalUnsupported` file naming inconsistency, `[currentLocator]` stale capture). Codex recommends splitting into #48a (resolver/registry/open/snapshot — smaller blast radius) + #48b (per-format settle/eval/host wiring — bigger refactor). Awaiting user decision: (A) accept split, (B) override cap and apply Round-3 fixes manually as v4, (C) defer feature. Plan at `dev-docs/plans/20260503-feature-48-debugbridge-probe-completion.md` carries the full audit log. |
