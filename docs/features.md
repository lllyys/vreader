...
| 48 | DebugBridge probe completion — settle hooks, live eval, richer snapshots, fixture path | DevTools/* | High | TODO | Phase-1 prerequisite for feature #45 (split out 2026-05-03 after Codex audit). Required work: per-format `settle` hooks (TXT TextKit, EPUB/Foliate WKWebView, PDF, TTS state) instead of the current 100ms placeholder; wire live `eval` evaluator into reader probes; snapshot schema additions (TTS state + current sentence, render phase, selection, settings provenance); test-fixture import path so test-only fixtures don't bloat Release; expand `DebugFixtureCatalog` beyond the single `war-and-peace` fixture; document container Reader → DebugReaderRegistry registration. See feature #45's audit notes for the gap list. |
