...
| 5  | Search highlight auto-dismiss on next action                  | Search/*     | Low      | DONE      | `searchHighlightClear` notification posted by `SearchViewModel`; observed by EPUB/TXT/PDF coordinators which dismiss the temporary highlight. 25 unit tests pass (`SearchHighlightDismissTests`, `ReaderAuditFix3Tests`). Tracker said "code committed, unchecked on device" but the wiring is complete and tested. Status promoted from TODO → DONE on 2026-05-03. Device verification listed as a manual-checklist follow-up. |
...
| 36 | OPDS catalog support                                          | BookSource/* | Medium   | DONE      | OPDSParser + OPDSModels services + OPDSCatalogListView/OPDSEntryView/OPDSBrowserView wired (C04). 24 unit tests pass (`OPDSParserTests` — feed parsing, entry extraction, link resolution, content/summary, deduplication, codable round-trip, Unicode). Status promoted from TODO → DONE on 2026-05-03. Device verification (live OPDS catalog browse + book download) listed as a manual-checklist follow-up. |
...
| 45 | Verification harness sweep — retire the "Needs device verification" backlog | DevTools/* | High | TODO | **Re-scoped 2026-05-03 after Codex audit of v1 plan**: was sized assuming DebugBridge (#44) was complete enough to drive verification. Audit revealed `settle` is a 100ms placeholder, `eval` has no live evaluator, snapshot lacks fields for TTS/render-phase/selection — the verification surface itself is incomplete. Phase-1 prerequisite split out as feature #48. This row now scopes to **Phase 2 only**: verification suite (XCUI flow tests + service-level integration tests + tracker drift script) + CI baseline. Re-plan after #48 lands. Depends on #44 + #48. GH: #147 |
| 48 | ~~DebugBridge probe completion (umbrella)~~ — split into #48a + #48b after 3 Codex audit rounds | DevTools/* | High | DUPLICATE | Split 2026-05-03 per `.claude/rules/47-feature-workflow.md` audit cap. Original plan at `dev-docs/plans/20260503-feature-48-debugbridge-probe-completion.md` retained for historical context. See #48a + #48b. |
| 49 | DebugBridge probe completion (a) — registry / open / snapshot wiring | DevTools/* | High | DONE | Phase-A shipped end-to-end: WI-0 grammar reconciliation (PR #176, v3.11.11), WI-1 DebugSnapshot schema v2 (PR #177, v3.11.12) — adds ttsState/ttsOffsetUTF16/settingsProvenance + RenderPhaseValue/TTSStateValue/SettingsProvenanceValue wire-value enums + DebugReaderProbe v2 default-impl accessors + TTSService.State.publicName, WI-7a DebugPositionResolver + awaitReader (PR #180, v3.11.15) — pure resolver per format + token-based waiter ownership covering the 5-case race matrix, WI-7b open() rewrite (PR #181, v3.11.16) — book-lookup → position-parse → unified-mode-guard → notification → awaitReader, surface fixes from Round-3 audit applied (CheckedThrowingContinuation, weak activeReader, BookRecord.fingerprint.format typing). Per-format seek dispatch (host-side seekStrategy) carved out to feature #50. Pending Gate 5 device verification (full `vreader-debug://open?bookId=...&position=...` round-trip) before status moves to VERIFIED. GH: #171 |
| 50 | DebugBridge probe completion (b) — per-format settle hooks + live eval + host wiring | DevTools/* | High | TODO | Phase-B of the split: per-format `settle` hooks (TXT TextKit layout, EPUB/Foliate WKWebView render, PDF page load, TTS state) replacing the current 100ms placeholder; live `eval` evaluator wired into each reader bridge via DEBUG-only `debugProbe: DebugReaderProbeAdapter?` host parameters; expanded `DebugFixtureCatalog` with multi-format fixtures. Depends on #49 shipping the registry/open/snapshot foundation. Plan TODO — write fresh once #49 lands and we have the post-#49 surface to plan against (the host-injection API design is the part that took 3 rounds; doing it in isolation will be cleaner). |
| 46 | WebDAV backup includes book files — materializing restore reconstructs library | Backup/* | High | VERIFIED | Phase 1 of two-feature split. Data layer end-to-end: `library-manifest.json` + content-addressed blobs via temp+MOVE atomic publication + materializing restore. 12 PRs (#153–#164, v3.10.20→v3.11.0). 80+ new tests across 8 test files. Plan at `dev-docs/plans/20260503-feature-46-materializing-restore.md`. Gate 5 verification: 4 curl probes against live rclone WebDAV (PROPFIND 207, MOVE 201, MOVE Overwrite-F→412, content-addressed blob round-trip with PROPFIND-by-size readback) — log at `dev-docs/verification/feature-46.md`. UI polish (size-confirm dialog when missing-bytes > 100 MB, per-blob progress) deferred. Phase 2 = feature #47. GH: #144 |
| 47 | WebDAV restore — selective book picker + lazy-on-tap downloads | Backup/* | Medium | PLANNED | Phase 2 of feature #46. `BookFileState` (`local / remoteOnly / downloading / failed / missingRemote`), library rows with cloud icons for un-downloaded books, "Restore selectively…" picker, lazy on-tap fetch via `URLSessionConfiguration.isDiscretionary` background sessions, Wi-Fi-only cellular policy. GC carved out to #51. Plan **v3** at `dev-docs/plans/20260503-feature-47-selective-picker-lazy-load.md` is Gate-3 ready — 2 Codex audit rounds applied (Round 1 SPLIT, Round 2 REVISE-with-list, both fixed). 9 WIs sequenced; major design decisions: `BackupBlobStore` ownership keeps state-aware manifest emit in `WebDAVProvider`, not collector; nonisolated `LazyDownloadDelegate` forwards to `@MainActor LazyDownloadCoordinator`; `BackgroundDownloadSessioning` protocol replaces concrete-URLSession factory; `LazyDownloadTaskMeta.schemaVersion` for forward compat; authenticated download requests built via new `WebDAVDownloadRequestBuilder`. Depends on feature #46 (VERIFIED in v3.11.1). GH: #145 |
| 51 | "Clean unused remote files" — WebDAV blob garbage collection | Backup/* | Medium | TODO | Carved out of feature #47 on 2026-05-03 per Codex Round-1 audit. Walks every backup manifest on the server, computes the union of referenced blob paths, deletes unreferenced blobs. Race-condition concern: another device backing up DURING our GC run could publish a blob we're about to delete. Codex flagged the v1 plan's "60-second grace" as insufficient; needs a conservative age floor (24h+), explicit exclude of `uploads/tmp/`, and skipped-recent surface count. Plan TODO — write fresh once #47 lands and we have the post-#47 `BookFileState` data model to design GC against. |
