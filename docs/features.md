...
| 47 | WebDAV restore — selective book picker + lazy-on-tap downloads | Backup/* | Medium | PLANNED (v2 revision needed) | Phase 2 of feature #46. Introduces `BookFileState` (`local / remoteOnly / downloading / failed / missingRemote`), library rows for un-downloaded books with cloud icons, "Restore selectively…" picker against the manifest, on-tap blob fetch via `URLSessionConfiguration.isDiscretionary`, Wi-Fi-only cellular policy. **GC action carved out → feature #51 per Codex round-1 audit (race-safety needs its own design pass).** Plan v1 at `dev-docs/plans/20260503-feature-47-selective-picker-lazy-load.md`; Codex Round 1 returned SPLIT verdict with 11 findings (manifest policy for non-local books, Share read-path missed, lazy-download `@MainActor` delegate compile-hostile, background URLSession lifecycle under-specified, ZIPWriter API mismatch, others). Plan v2 must address those before Gate 3. WI count likely grows to 9-10 after de-cramming. Depends on feature #46 (VERIFIED in v3.11.1). GH: #145 |
