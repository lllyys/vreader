...
| `vreaderTests/Services/PersistenceActorReadingWindowTests.swift` | Actor tests for `sumReadingSeconds(in:)`. | sums sessions inside the interval; excludes sessions outside it; boundary sessions counted by `startedAt`; negative durations clamped; empty store → 0; large-history sum does not overflow. |
