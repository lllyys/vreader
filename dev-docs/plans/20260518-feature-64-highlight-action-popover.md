...
- **Status**: **BLOCKED at Gate 2 — round 3 = NEEDS-REVISION (escalated to user).** Round 3 (Codex `019e3f4d`) found 1 Critical + 2 High. The Critical is structural: feature #55 (note-preview, shipped) has already re-homed tap-on-highlight on **every** format, so the plan's whole premise — "tapping a highlight shows a bare Delete UIMenu on 5 live presenter call sites" — is false against HEAD. Gate-2's 3-round cap is reached with an unresolved Critical; per rule 47 this STOPS and escalates. See §0.1.
...
| — | 2026-05-19 | **Gate-2 round 3 = NEEDS-REVISION (Codex `019e3f4d`). BLOCKED — escalated to user.** Round 3 found 1 Critical + 2 High. The Critical invalidates the plan's foundational premise: feature #55 already shipped. The 3-round Gate-2 cap (rule 47) is reached with the Critical unresolved → STOP + escalate. The plan is NOT re-revised to v4; it awaits a user decision on re-scope/defer/redesign. See §0.1. |
...
## 0.1. Gate-2 round-3 audit — BLOCKER (Codex `019e3f4d`, 2026-05-19)

Round 3 was an independent re-audit of plan v3. It found **1 Critical + 2 High**. Each finding was independently re-verified against HEAD by the feature-workflow agent (greps + file reads) before being recorded — all three confirmed. Gate-2's 3-round cap (rule 47) is reached with a Critical open, so per rule 47 the workflow **STOPS at Gate 2 and escalates to the user**. The plan is intentionally NOT bumped to v4: a v4 would be guessing at a re-scope the user must own.

| # | Sev | Round-3 finding | Verified-against-HEAD evidence |
|---|-----|-----------------|--------------------------------|
| R3-F1 | **Critical** | The plan is built on a deleted interaction model. **Feature #55 (note-preview, shipped) has already re-homed tap-on-highlight on every format.** The plan's whole premise — "tapping a highlight shows a bare Delete `UIMenu`; 5 live `presenter.present` call sites to migrate; Foliate WI-5 migrates a Foliate presenter" — is false against HEAD. | Confirmed. **EPUB**: `EPUBWebViewBridgeCoordinator.handleHighlightTapMessage` (`EPUBWebViewBridgeCoordinator.swift:125-131`) posts only `.readerHighlightTapped`; its own doc (`:112-117`) says the tap "now opens the #55 note preview … The legacy #53 tap-time inline delete menu" is gone. `EPUBReaderContainerView.swift:473` documents the bridge "no longer takes a `highlightActionPresenter` / `onHighlightTapAction`." **Foliate**: `FoliateSpikeView+HighlightTap.swift` header — "Feature #55 WI-7: a tap on an annotated AZW3/MOBI highlight now opens the #55 note preview … there is **no `present(...)` call** to re-home." No `performDelete`, no `highlightActionPresenter` on `FoliateSpikeView`. `ReaderContainerView.swift:685` documents Foliate "no longer takes `highlightActionPresenter`." **The `.present(for:` call sites that DO survive are 3, not 5, and they fire on a deliberate LONG-PRESS, not a tap**: `TXTTextViewBridgeCoordinator.swift:235` (`handleHighlightLongPress`, doc `:217-223`: "Feature #55 WI-6: re-homes feature #53's inline delete menu from a tap to a long-press"), `TXTChunkedReaderBridge.swift:494`, `PDFViewBridge.swift:480`. Implementing v3 as written would (a) regress shipped #55 behavior, (b) spend WI-5 + parts of WI-4 migrating EPUB/Foliate paths that no longer exist. |
| R3-F2 | High | WI-1 adds the wrong read API. The codebase **already has** a scoped single-highlight lookup boundary — `HighlightLookup.highlight(withID:forBookWithKey:)` (`HighlightLookup.swift:22`, feature #55 WI-1), implemented at `PersistenceActor+Highlights.swift:146`, already consumed by `NotePreviewViewModel.swift:37`. The plan's claim "there is currently no fetch-by-ID method" is false; its proposed unscoped `HighlightPersisting.fetchHighlight(highlightId:)` duplicates it and **drops the `(id, bookKey)` cross-book-leak safety contract** for no reason. | Confirmed. `HighlightLookup.swift` exists; `protocol HighlightLookup: Sendable { func highlight(withID:forBookWithKey:) async throws -> HighlightRecord? }`. `PersistenceActor+Highlights.swift:146` implements it with a `#Predicate` on `highlightId` **AND** `book?.fingerprintKey`. `NotePreviewViewModel` already depends on `any HighlightLookup`. Any rebuilt plan must reuse `HighlightLookup`, not invent a second, weaker lookup. |
| R3-F3 | High | The plan's own Gate-2 round-1/2 audit trail is untrustworthy. v3's §0 tables assert F1/F8/R2-F3 were "verified" because Foliate "still takes `highlightActionPresenter`, still calls `present(...)`, still has `performDelete`." **None of that is true at HEAD** — feature #55 removed those hooks. The round-history claims describe pre-#55 history as if it were current fact. | Confirmed. `FoliateSpikeView.swift:280` documents the JS-delete observer as a **dormant hook awaiting a future producer**, not a live path. The F1/F8/R2-F2/R2-F3 "verified against `FoliateSpikeView.swift:28,67` / `FoliateSpikeView+HighlightTap.swift:55,89,112-134`" lines in §0 no longer match the files at those paths/lines. |

### Why this blocks, and what the user must decide

R3-F1 is not a fixable detail — it changes what feature #64 *is*. Possible directions (the user picks; do not pick autonomously — this is a re-scope, not a finding to patch):

1. **Re-scope #64 to the surviving surface only.** The bare-`UIMenu` delete *does* still exist — on the **TXT/MD/PDF long-press** (3 call sites). #64 could become "skin the long-press delete `UIMenu` into the styled `HighlightActionPopover` for TXT/MD/PDF." But: this drops EPUB + AZW3/MOBI (no presenter there), and the design's `HighlightActionPopover` has color-change + note-edit + share — far more than a delete menu — so even the long-press surface only partially matches the design. And tapping any highlight already opens the #55 note preview, which itself overlaps the popover's note-display role.
2. **Re-scope #64 as an extension of the #55 note-preview surface.** Tap already opens note preview on every format. #64's `HighlightActionPopover` (color row + note edit + copy/share/delete) could be folded into / replace the note-preview sheet. This is the most design-faithful reading, but it is a materially different feature from the row's current text ("replace the bare Delete UIMenu") and overlaps #55's shipped surface — it needs a fresh problem statement and a check of whether it supersedes part of #55.
3. **Defer / close #64 as substantially superseded by #55.** Feature #55's note preview already delivers tap-on-highlight → styled sheet. If the user considers #64's intent (a styled surface on highlight interaction) effectively met by #55, #64 may be `DEFERRED` or `WONT DO` with a note, or narrowed to just the unbuilt deltas (color-change row, in-place note edit).

Whichever direction: the plan must be **rewritten from §1 down** against HEAD (correct call-site inventory, `HighlightLookup` reuse, the real #55 note-preview surface, a corrected WI list — almost certainly fewer than 6 WIs, no Foliate-presenter migration), and the row text in `docs/features.md` updated to match. Then Gate 2 restarts at round 1 on the rewritten plan.

**Feature-workflow status at escalation**: row #64 stays `TODO` (it never reached `PLANNED` — Gate 2 never passed). No branch, no code, no GH-issue state change. GH #822 already exists (created when the row was first mirrored); it should get a comment noting the Gate-2 block so the public handle reflects reality.

## 1. Problem (as drafted — see §0.1: premise invalidated by feature #55)
