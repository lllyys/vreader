...
| `reader/details/BookDetailsRows.kt` | `BookActionList` gains two **nullable** callbacks `onExportAnnotations: (() -> Unit)? = null`, `onImportAnnotations: (() -> Unit)? = null`; a null one renders **no row** (capability pattern, no dead no-op — the `onJumpToAnnotation` precedent at `AnnotationsReviewSheet.kt:117`). Rows follow the shipped Share row's geometry (`:240-279`). **Plus B1's merge-policy footnote caption** under the card when either callback is non-null (§3.3 A-4). Tags `details-export-annotations`, `details-import-annotations`, `details-annotations-footnote`. Update the file's KDoc at `:228` (rule 22 — it currently says "Share ONLY"). |
...
**Export — `AnnotationsIoController.export(uri, …)`:** identical treatment. `openOutputStream` is
bounded with `dispose = { it?.close() }`; the write is bounded; a `TimedOut` maps to a typed
failure. Round 1 did not bound the export path at all — a hostile provider could park the reader
activity on a *save*, which is if anything the easier attack to trigger (the user chose "Export").
...
| **A-12** | **A hostile provider cannot wedge the reader** (§8.1) | a test with a fake resolver that returns promptly — it never exercises the parked path at all | A fake `ContentResolver` whose `query` (case 1) / `openInputStream` (case 2) / `read` (case 3) **blocks forever**. Assert for each: the controller **returns a typed `ImportFailure` within the timeout budget**, `gate.abandonedCalls` incremented, and — after the fake is released — the count returns to 0 (self-heal). Plus a **late-result disposal** case: the fake returns a stream *after* the deadline; assert `close()` was called on it (no fd leak). Same three cases for `export`'s `openOutputStream`. |
...
| `AnnotationsIoControllerTest.kt` | **A-12** — the hostile-provider suite, JVM, no emulator: a fake `ContentResolver` parking in `query` / `openInputStream` / `openOutputStream` / `read`; typed failure within budget; `abandonedCalls` increments then self-heals; late-result stream is `close()`d; `Busy` when the admission budget is exhausted; export's bounded path symmetrical to import's |
...
| R-3 | A hostile **provider** parks the reader activity — not just a hostile *file* | §8.1: every `ContentResolver` call (query, open, transfer, **both directions**) goes through the shipped `BoundedCallGate` with mandatory late-result disposal; A-12 proves it with a fake that parks in each of the four call sites. Honest caveat preserved: caller-liveness is **guaranteed**, unparking the thread is **best-effort**. |
