...
| v3 audit | 2026-05-19 | **Round-3 Gate-2 confirming audit (Codex `019e3e3a`) — verdict CLEAN: zero open Critical/High/Medium.** The auditor re-verified every live-code assumption v3 depends on (`FormatCapabilities.capabilities(for:)`/`case .azw3`, `ReaderAICoordinator.loadBookTextContent`'s `default: return nil`, `FoliateSpikeView.Coordinator`'s `weak webView`/`isBookReady`/`makeCoordinator()`/`layout-ready` flip, `startTTS()`/`ensureAIReady()` and that `startTTS()` is NOT `@MainActor`-annotated, `foliate-host.js`'s `readerAPI`+`bookReady`/`currentBook`, the `createDocument()` whole-book seam, `build-bundle.sh`) and confirmed all sound. The 3-layer idempotency + ~12s timeout closes the prior MEDIUM. **Two LOW-only notes, both accepted (not blocking) and recorded here:** (L1) §3.1/§3.4/§5/§11 still name the dispatcher seam `ReaderContainerView.nativeReaderView` / `case "azw3"`, but feature #54 changed the live dispatcher to `engineReaderView` → `ReaderEngine.resolve(format:) -> .foliateWeb` → `FoliateSpikeView(...)` (`ReaderContainerView.swift:613-658`). The implementation is mechanically identical — pass `coordinatorBox:` to that same `FoliateSpikeView(...)` initializer; WI-1/WI-2 use the live `case .foliateWeb:` seam. (L2) §3.1a/§9-criterion-4's "stop is still possible via the control bar during extraction" is inaccurate — `TTSControlBar` only renders once `ttsService.state != .idle`, so a stop *during* extraction (state still `.idle`) is not reachable via the control bar; the `ttsService.state == .idle` re-check after the extraction `await` is still correct defensive code (a `vreader-debug://tts?action=stop` or reader-dismiss can still change state mid-walk), but the rationale wording is corrected to cite the debug-stop / dismiss path rather than the control bar. Gate-2 exits clean at round 3. |
