...
| WI-5 | **EPUB reader host + open**: Readium `EPUBNavigatorFragment` (scroll mode, per Spike B) in a Compose host; open the stored publication from app-private storage. Instrumented open test on the emulator. | behavioral | Medium |
| WI-6 | **Resume + process-death (Gate-2 Critical + Medium)**: on `locationDidChange` save the `VReaderLocator` envelope (Readium `Locator` JSON + canonical fallback) → Room. Restore **precise-first / canonical-fallback-second**. Acceptance bar = the **Spike-B-proven window** (resource/progression precision; scroll-mode `currentLocator` is ~2-paragraph-coarse on CJK, NOT within-chapter exact — do not over-claim). Tests: close→reopen AND **config-change / process-death** recovery of current book + last position via route/book-id state + Room reopen (rule 10 Android edge cases). | behavioral | Large |
