...
**Status**: Gate 2 PASSED → WI-1 implemented (Gate-4 found paged needs WI-2)
→ 2-WI split
...
- **WI-2 — justify paged-mode page-terminal lines** — *behavioral, FINAL WI*.
  ...
  page (Gate-4 Medium). Fix: render pages from a SHARED TextKit layout context
  (the paginator already lays out the full chapter with one `NSLayoutManager` +
  per-page `NSTextContainer`s) so page-internal lines — including the
  page-bottom line — are justified; only true paragraph-terminal lines stay
  natural. Pure code (no new UI → rule 51 N/A). Its own RED test (a paged page
  that breaks mid-paragraph has a justified bottom line). PR → row `DONE`.
  Minor bump (completes the feature).
...
| **Paged-mode page-bottom line ragged** (page breaks mid-paragraph → the isolated `attributedSubstring`'s terminal line isn't justified) | Certain in paged mode (Gate-4 Medium) | **Owned by WI-2** — render paged pages from a shared TextKit layout so page-internal lines (incl. page-bottom) justify. WI-1 ships scroll + chunked (the user's case); paged is a strict improvement under WI-1 (most lines justified) and fully correct after WI-2. |
