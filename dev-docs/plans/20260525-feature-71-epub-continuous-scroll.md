...
- **v4.4 (2026-05-27)** — WI-6b-iii shipped as **position restore only** (Codex thread `019e6861`, 2 rounds, ship-as-is). The coordinator gains `restoreFraction` (from `currentPosition.progression`) and scrolls the anchor section to it after the initial window materializes. The other two 6b-iii parts were **investigated and resolved without code here**: (a) **inner-scroll-root safe-area (finding 4)** — device-verified that the inherited Bug-#163 `webView.scrollView` inset ALREADY positions continuous content below the notch (`eval firstSectionTopY: 76`, screenshot not clipped); an initial `margin-top` bootstrap inset was non-functional (`safeAreaTopInset=0` at bootstrap-load) + a double-inset risk, so reverted; bottom-clip edge (root `height:100vh` while inset) noted as a follow-up; (b) **live mode-switch rebuild (finding 2 full)** — finding-2 SAFETY is already met by 6b-i's `invalidate()`+config-release on the one-way hard-block, so the full live rebuild is deferred (low priority for a dark feature). Gate-4 round-1 caught a stale-generation gap between the two initial-window extends (fixed with a between-extends guard). Audit log `.claude/codex-audits/feat-feature-71-wi-6b-iii-restore-safearea-audit.md`. Remaining for the feature: WI-7 (bilingual), WI-8 (final + docs + flip the flag default); the deferred bottom-clip + live-rebuild can fold into WI-8.
