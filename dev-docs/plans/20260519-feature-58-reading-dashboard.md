...
| v4 | 2026-05-20 | **D-answers A/B/B/A applied** ([GH #665 comment 4497841858](https://github.com/lllyys/vreader/issues/665#issuecomment-4497841858)). D1=A: SettingsView profile-card entry (accepts feature #67 dependency); D2=B: Custom range picker deferred to GH #1058 needs-design; D3=B: `last-read` 5th column deferred to GH #1059 needs-design; D4=A: hero + pill-bar layout per design. **WI-6 splits into 3 sub-WIs**: WI-6a (A-portion: `ReadingDashboardView` + `StatsTimeWindowBar` 6-pill bar + `StatsPerBookTable` 4-col + DEBUG-only `vreader-debug://stats` entry — ships now, **user-facing SettingsView Stats button hook deferred to feature #67 WI-4** which is itself hard-blocked on this WI-6a reaching `DONE`); WI-6b (D2-B Custom range picker + `Year` calendar-YTD bucket swap, blocked on GH #1058); WI-6c (D3-B `last-read` sort column, blocked on GH #1059). WI-6a's pill bar renders the existing 7 enum cases — under D2-B with `Year` semantics deferred to WI-6b, this is the row's rolling-day labels (Today/7d/30d/90d/180d/365d/All); WI-6b swaps `last365Days` → calendar-YTD `year` once the picker design clarifies the bucket boundary. |
| v5 | 2026-05-20 | **WI-6c SHIPPED** — `last-read` 5th sortable column landed once GH #1059's `stats-followups-artboards.jsx` design bundle landed (PR #1060). `.lastRead` case added to `ReadingDashboardSortField`; comparator sinks nil-`lastReadAt` rows to the bottom regardless of direction (per design); `StatsPerBookTable` gained a 5th header-tap-driven `Read` column (Alt-1 always-5-columns design variant; chosen over the canonical sort-menu variant because the existing 4 columns already use header-tap sorting — a separate sort-menu surface would have been a larger redesign than D3-B's scope). Nil cells render as "—". `ReadingTimeFormatter.formatRelativeLastRead` already existed (feature #60 WI-8) and is reused for non-nil cell text. WI-6b (D2-B Custom range picker) is the last remaining pre-WI-8 sub-WI. |
