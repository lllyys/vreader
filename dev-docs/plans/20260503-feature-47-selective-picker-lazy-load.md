...
| 4a | behavioral | `BookFileImportFinalizer` extraction from `BookFileMaterializer`. Existing materializer tests must still pass; new finalizer tests. `RemoteBookCatalog` (using correct `ZIPWriter.extractEntry(named:from:)` API). **WI-3a follow-up:** enqueue path must serialise downloads per `fingerprintKey` (or thread `taskIdentifier` through coordinator state) so a stale callback from an older same-key task can't be accepted into a new attempt after `prepareToDownload(fingerprintKey:)` clears `terminalKeys`. | 3 prod (mod+new), 2 test | small-medium |
