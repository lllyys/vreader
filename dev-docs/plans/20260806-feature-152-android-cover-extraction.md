...
| `MobiCoverRealBookTest` | **connected, CI-safe** | `androidTest/assets/foliate-spike/book.azw3` yields the correct cover — **record 135, 379,691 bytes, 542×800**, digest-pinned. *Fixture note (audit L-13, corrected):* this asset is **byte-identical** to the gitignored real book (both `md5 f4ae9259b82cc2a765242bebd015df84`, 6,288,371 bytes — verified), so the real-book happy path is already committed, needs **no per-run `adb push`**, and runs in CI. The earlier "push the real book every run" instruction was over-pessimistic and is withdrawn. The structural failure modes still cannot come from any real book (a truncated header or an out-of-range EXTH index has to be constructed) → synthetic in-test buffers, AGENTS.md "deterministic tiny structure" exception. |
