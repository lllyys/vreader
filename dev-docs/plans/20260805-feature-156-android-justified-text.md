...
| **R4** | **`publisherStyles = false` changes far more than alignment** — line-height starts applying (W11 → bug #367), the advanced type scale engages, paragraph font sizes flatten to `1rem` | **Certain**, by construction | **Bounded and enumerated in §7.2 (E1–E5)**, not left as a vague "blast radius": the gated properties we never set (`paraSpacing`, `paraIndent`, `wordSpacing`, `letterSpacing`, `bodyHyphens`) emit no variable and cannot fire, so exactly five effects are possible and each has its own computed-style verification. **CORRECTED 2026-08-06 by WI-2's implementation: the enumeration was INCOMPLETE — there are seven, not five.** Re-derived from the shipped stylesheet and measured, WI-2 found **E1b** (the publisher's `text-align` on `p`/`li` is overridden — a centred `<p>` becomes `justify` — and `text-align-last` is forced to `auto`, while `blockquote`/`figcaption` prose keeps the publisher's) and **E1c** (the rule targets `:root`, so **headings inherit justification**). The plan mentioned the mechanism in prose at §4.2 and §5.3 but never listed either as a verified effect. Both are now pinned as executable characterisations. The reasoning that bounded the set was sound; the enumeration derived from it was not exhaustive — which is why WI-2's brief required the list to be *verified* rather than inherited. WI-2 is its own PR, own audit, own Gate-5 slice on **both** EPUBs with before/after screenshots. §7.2 also argues why the flag cannot be split into its own feature (`textAlign` is inert without it). Disclosed in the PR body + release note as "EPUB typography that was silently inert now applies". |
